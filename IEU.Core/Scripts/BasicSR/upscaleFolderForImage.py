...
def main():
    ...
    for test_loader in test_loaders:
        ...
        for data in test_loader:
            ...
            sys.stdout.flush()
            if not os.path.exists('{1:s}/Images/{0:s}/'.format(baseinput, dataset_dir)):
                os.makedirs('{1:s}/Images/{0:s}/'.format(baseinput, dataset_dir))
            util.save_img(sr_img, '{3:s}/Images/{0:s}/[{2:s}]_{1:s}.png'.format(baseinput, img_name, modelname, dataset_dir))
            print(idx, img_name)
            sys.stdout.flush()
