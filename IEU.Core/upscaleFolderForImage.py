...
for path in files_i_care_about:
    ...
    if not os.path.exists('results/Images/{:s}/'.format(baseinput)):
        os.makedirs('results/Images/{:s}/'.format(baseinput))
    cv2.imwrite('results/Images/{0:s}/[{2:s}]_{1:s}.png'.format(baseinput, base, modelname), output)
    print(idx, base)
    sys.stdout.flush()
